...
def prepare_data():
    ...
    x = np.zeros((len(user_appreciates), 10, 4096))
    y = np.zeros((len(user_appreciates), 10, 4096))
...
decoder_input = keras.Input(shape=(SEQUENCE_LENGTH, 4096))
encoder_input = keras.Input(shape=(SEQUENCE_LENGTH, 4096))
x = decoder(decoder_input, encoder_input)
output = keras.layers.Dense(4096, activation = "relu")(x)
