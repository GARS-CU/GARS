...
def export(dataset):
    ...
    open_features = np.zeros((len(labels), 10, 329))
    emotion_features = np.zeros((len(labels), 10, 48, 48, 1))
    ...
    np.save(os.path.join("..", "..", subset, "emotion_features.npy"), emotion_features)
    np.save(os.path.join("..", "..", subset, "open_features.npy"), open_features)

    np.save(os.path.join("..", "..", subset, "labels.npy"), labels)


for data in ["val", "train", "test"]:
